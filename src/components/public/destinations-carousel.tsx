    ...
    return (
        ...
        >
                    ...
                    <CarouselItem key={dest.id} className="pl-4 md:basis-1/3 lg:basis-1/4">
                        <div className="relative group cursor-pointer overflow-hidden rounded-3xl h-80 shadow-lg ring-1 ring-black/5">
                            ...
                            <div className="absolute bottom-0 left-0 p-6 text-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                ...
                                <p className="text-sm text-stone-200/80 font-medium">{dest.tourCount}</p>
                            </div>

                            {/* Hover top tag */}
                            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <span className="bg-amber-500 text-stone-950 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow">
                                    Explore
                                </span>
                            </div>
                        </div>
            ...
            <div className="hidden md:flex justify-end gap-2 mt-5 pr-12">
                <CarouselPrevious className="relative translate-y-0 translate-x-0 bg-white border-stone-200 hover:bg-stone-50 text-stone-700 hover:text-stone-900 shadow-sm" />
                <CarouselNext className="relative translate-y-0 translate-x-0 bg-white border-stone-200 hover:bg-stone-50 text-stone-700 hover:text-stone-900 shadow-sm" />
            </div>
        </Carousel>
    );
}
